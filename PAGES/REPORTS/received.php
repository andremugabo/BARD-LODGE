...
<div class="container-fluid section-title d-flex">
    <div class="s-title text-start col-6">
        <h2>Sales Received Products On 2022-03-13</h2>
    </div>
    <div class="s-btn text-end col-6">
        ...
        <button type="button" class="btn btn-sm btn-danger"
            onclick="window.location.href='receivedReport.php'">BACK</button>
...
<div class="col">
    <div class="card">
        <div class="card-header">
            <caption>List of Received Sales Products</caption>
        </div>
        <div class="card-body overflow-auto">
            <table class="table align-middle mb-0 bg-white table-striped table-sm table-hover">
                ...
                $s_id = $_GET['s_id'];
                $receivedObj->setSId($s_id);
                $dailyReceived =$receivedDao->selectReceivedBySId($receivedObj);
                // print_r($dailyReceived);
                $num = 0;
                $total = 0;
                $sum = 0;
                if($dailyReceived != null):
                    foreach($dailyReceived as $item){$num++;

                    $total = $item['QTY_REC'] * $item['P_PPRICE'];
                    $sum+=$total;
