        ...
        <div class="card-body overflow-auto">
            <table class="table align-middle mb-0 bg-white table-striped table-sm table-hover">
               ...
               $selectUsers =$user->selectUsers();
                $num = 0;
                if ($selectUsers):
                foreach ($selectUsers as $items) {  $num++;   
                    // print_r($item);         
                ?>

                    <tr>
                        <td style="text-align: center;"><?=$num?></td>
                        <td style="text-align: center;"><?=$items['FIRSTNAME']." ".$items['LASTNAME']?></td>
                        <td style="text-align: center;"><?=$items['E_ROLE']?></td>
                        <td style="text-align: center;">

                            <button type="button" class="btn btn-success btn-sm mb-1"
                                onclick="window.location.href='byEmployeesMetric.php?id=<?=$items['E_ID']?>'">View&nbsp;Metrics</button>&nbsp;
                        </td>
                    </tr>
